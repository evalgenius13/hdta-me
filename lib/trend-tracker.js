...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Generate trend context locally (no API call)
    const trendTracker = new TrendTracker();
    const policyData = trendTracker.trackPolicy(article, basicAnalysis);
    const trendContext = trendTracker.generateTrendContext(article, basicAnalysis);
