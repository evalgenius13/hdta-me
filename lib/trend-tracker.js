...
class TrendTracker {
  ...
  formatTrendContext(trends) {
    ...
    if (trends.patterns.recentSimilar > 0) {
      context += `This is Trump's ${trends.patterns.recentSimilar + 1} policy targeting ${trends.patterns.category} in recent weeks. `;
    }
    
    // Velocity context
    context += `Trump is averaging ${trends.velocity.policiesPerWeek} policies per week. `;
...
async function generateDashboard(timeframe, category) {
  ...
  return {
    ...
    pattern_analysis: {
      ...
      reversal_rate: "73% of policies reverse Biden-era rules",
...
**Pattern Analysis:** This fits Trump's broader strategy of ${trendData.patterns.category} deregulation, with ${trendData.patterns.recentSimilar} similar policies recently.`;
...
async function generateBasicAnalysis(article) {
  ...
  const prompt = `You're explaining this Trump administration news to a friend who wants to know what's really going on.
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You explain Trump administration policies like a smart friend who sees through political spin. You use simple words and show who really benefits vs who pays the price.'
        },
        {
          role: 'user',
          content: prompt
